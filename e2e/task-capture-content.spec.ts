...
import {capture,createTask,inbox,openApp,openTask,readTasks} from './planner-helpers';
for(const width of [1280,390]){
 test(`captures literal title, content and steps atomically at ${width}px and retains a closed draft across reload`,async({page},testInfo)=>{
    ...
    await sheet.getByText('Context and notes',{exact:true}).click();
    ...
    const draft=await capture(page,'Unfinished draft');await draft.getByText('Context and notes',{exact:true}).click();await draft.getByRole('textbox',{name:'Content',exact:true}).fill('Do not lose this thought');
    ...
    await draft.getByText('Context and notes',{exact:true}).click();await expect(draft.getByRole('textbox',{name:'Content',exact:true})).toHaveValue('Do not lose this thought');
