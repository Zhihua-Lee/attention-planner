...
export const ListView = memo(function ListView({ title, statusFilter, recurringOnly = false }: ListViewProps) {
    ...
    }, [baseTasks, metadataAreaById, metadataProjectMap, recurringOnly, resolvedAreaFilter, statusFilter]);
    ...
    useEffect(() => {
        ...
        const cacheKey = `${statusFilter}-${recurringOnly ? 'recurring' : 'all'}-${lastDataChangeAt}`;
    ...
    }, [statusFilter, recurringOnly, queryTasks, getDerivedState, lastDataChangeAt, showToast, tasks]);
    ...
    const filteredTasks = useMemo(() => {
        ...
        return perf.measure('filteredTasks', () => {
            ...
            const filtered = deferredFilterInputs.baseTasks.filter(t => {
                ...
                if (deferredFilterInputs.recurringOnly && !t.recurrence) return false;
    ...
    const showFilters = !recurringOnly && ['next', 'all', 'done', 'waiting', 'someday'].includes(statusFilter);
    ...
    const emptyState = (() => {
        if (recurringOnly) {
            return {
                title: resolveText('plan.recurringEmpty', 'No recurring tasks'),
                body: resolveText('plan.recurringEmptyHint', 'Add recurrence while editing a task; its next occurrence will be generated by the existing recurrence engine.'),
            };
        }
        switch (statusFilter) {
            case 'inbox':
