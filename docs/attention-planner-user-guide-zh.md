...
今日承诺复用焦点星标，表示“我今天承诺关注它”，不是永久优先级。只有已经可用的 Ready 任务能成为今日承诺；任务改成 Waiting、Someday 或 Inbox 时，系统会自动取消星标，避免它继续占用今天。建议每天 1–3 项，状态不好时只保留 1 项。
...
如果周期任务只设置“以后可用”而没有精确安排或截止日期，Recurring 仍会显示下一次可用日期预览；它不会出现在 Calendar 上，因为 Available 不是日历时间块。
