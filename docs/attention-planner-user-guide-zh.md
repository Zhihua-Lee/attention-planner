...
- 卡片左上角勾选框用于完成任务，完成提示中的“撤销”可恢复误操作；右上角把手仍负责拖动或选择父任务。
- 点击任务标题打开详情，不再重复摆放“详情／安排”按钮。收集箱只保留简短的“加入待办”主操作。
- 待办卡片右下方的收集箱图标，或详情中的“移回收集箱”，可以重新整理任务。正文、步骤、独立子任务归属和原有计划保留；移回不是取消安排。需要释放时间时，在详情取消对应工作时段或当天意向。
- 垃圾桶图标直接移入回收站，提示中可撤销，不再经过只有一个选项的“更多”菜单。
- 有内容的步骤清单默认展开，标题显示完成数／总数；主动收起后，本次编辑不会因其他字段变化而重新展开。恢复有正文或步骤的录入草稿时，外层内容区也会展开。
...
- **内容**：Task 可直接组织独立子 Task，也可只附轻量 Checklist；Area／Project 都是可选归属，说明事情是什么；
...
误点完成可以恢复：NOW、日历和任务列表完成后，会显示至少 12 秒的「撤销」按钮；电脑在没有编辑文本时也可按 Ctrl/Cmd+Z 撤销最近一次操作。若在设置中关闭了撤销提示，快捷键仍有效。

提示消失或重新打开应用后，展开侧栏的 Archive（归档分组）→ Done（已完成），点击任务左侧常驻的「恢复为待办」回转箭头。任务会回到 Ready；正文、步骤和排程保留。即时撤销还会按今日承诺的数量上限恢复原有星标。注意：周期任务完成时生成的下一期是独立记录，恢复本期不会自动删除下一期。
...
- 只想记一句话：在列表快速输入标题，按回车添加。
- 想一次写完整：点击快速输入下方的 **正文与步骤**，已输入的标题会带入添加弹窗；也可以从侧栏“添加任务”打开，再展开“正文与步骤”。
- 在正文里写背景、链接或 Markdown 缩进列表；展开 Checklist 添加任务内部步骤。
- 点击“保存”才会把标题、正文、步骤一起创建为一个任务。关闭或取消弹窗不会留下任务；折叠正文区不会清空草稿。
- PWA 详情中的“编辑内容”会一起保存标题、正文和步骤；读详情时直接勾选步骤则即时保存。只有已经创建的任务才提供“提升步骤到收件箱”，不会在新建草稿中提前产生任务。

### 清单步骤与独立子任务：不必先建项目

想把几件事放在一起，并不要求你使用 Project：直接用一个父 Todo 组织子 Todo 即可。

- **轻量步骤（Checklist）**：文字＋勾选，没有自己的时间块或提醒。适合“带钥匙、带水杯”这样的检查项。
- **独立子任务**：是完整 Todo，有自己的正文、日期、提醒、完成状态和日历时间块；还可包含下一级子任务。

操作方法：

1. 已有两个 Todo：电脑抓住任务卡右上的把手，拖到另一个 Todo；手机长按把手再拖动。拖动只有把手占用手势，卡片其他区域仍可滚动。
2. 不想拖动：点把手，或打开详情 → **移动任务…**，搜索并选择父任务。键盘也能 Tab 到把手，按 Enter 打开选择器。
3. 新增子项：父任务详情 → **添加子任务** → 填标题 → **添加并打开**。随后补正文、截止日期或在“安排这件事”中**预留一个时段**。子任务可以有多个独立时段，会正常显示在 Calendar。
4. 移出：子任务详情 → **移动任务…** → **移出 · 成为顶层任务**。移动后的提示提供“撤销”，电脑还可使用 Ctrl/Cmd+Z（输入框中的撤销仍交给文字编辑）。

移动只改变归属，原内容和时间不变。完成或删除父任务**不会**自动完成／删除子任务；父任务不在当前列表时，子任务仍独立显示。父任务的时间不会自动分配给子项，也不是子项工时的合计。

原清单步骤若需要独立规划，可先“提升为独立任务”，再通过“移动任务…”放回父 Todo。普通步骤不会被偷偷变成完整任务。

### 电脑与手机填写小技巧

- 编辑标题、正文和步骤后，点击底部常驻的“保存全部修改”；电脑可用 Ctrl/Cmd+Enter 一起保存。中文输入法的确认回车不会额外保存或插入步骤。
- 手机表单的保存区随可视窗口缩小，给软键盘留出空间；长内容在表单内部滚动。保留系统缩放，不禁用双指缩放。
- 新建草稿、未完成的正文编辑和父任务内的新子任务标题会在当前设备保留；这些编辑草稿本身不跨设备同步。
- 添加层级功能后，请让电脑和手机 PWA 都刷新到新版再同步。Google Drive 同步和完整备份包含父子关系；CloudKit 尚不支持，上传会明确拒绝，不静默丢字段。
