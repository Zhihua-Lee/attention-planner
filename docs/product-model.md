...
Area（长期领域）
└─ Project（有明确终点的成果）
   └─ Task（可以直接执行的动作，也可独立于 Project）
      ├─ Child Task（可继续嵌套，独立的任务记录）
      └─ Checklist（任务内部轻量步骤）
```
...
- PWA 详情进入“编辑内容”后，标题、正文、Checklist 一起保存或放弃；只读详情中的勾选／步骤提升仍是即时操作。新建草稿直到保存才创建记录。
- `Task.parentTaskId` 是可选内容关系，可多层嵌套；不强制 Project，不复制父任务的正文、日期、提醒、状态或工作时段。移动已有任务只改这一个关系。父任务完成、归档或删除不会级联处理子任务，父任务恢复后关系仍可显示。
- 子任务拥有自己的 Available、Due、重复规则和工作时段，按自己的资格进入 NOW 和 Calendar。父任务的日期／预计工时不是子任务的合计，也不会自动分配给子任务。
- 自己和自己的后代不能成为父任务。离线并发造成的循环只在展示时按稳定 ID 断开，不改写原数据；每个任务仍可见并可移出。父任务缺失或不在当前筛选中，子任务按当前视图的顶层任务显示。
- 桌面拖动把手可移入另一任务；手机可长按把手拖动，也可点把手或详情中的“移动任务…”选择父任务／移出。移动提供撤销，键盘可通过同一选择器操作。原生手机编辑器不新增此界面，但共享存储保留关系。
